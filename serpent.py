...
class serpant:
    def __init__(self, key):
        # Преобразование ключа в шестнадцатеричный формат
        key = codecs.encode(key, 'hex').decode('utf-8')
    ...
    def encrypt(self, block):
        # Преобразование блока в битовую строку
        plainText = help_functions.convertToBitstring(
            help_functions.reverse(codecs.encode(block, 'hex').decode('utf-8').lower()), 128
        )
        cipherText = encrypt(plainText, self.userKey)
        # Преобразование зашифрованного текста обратно в байты
        return codecs.decode(help_functions.reverse(help_functions.bitstring2hexstring(cipherText)), 'hex')

    def decrypt(self, block):
        # Преобразование зашифрованного блока в битовую строку
        cipherText = help_functions.convertToBitstring(
            help_functions.reverse(codecs.encode(block, 'hex').decode('utf-8').lower()), 128
        )
        plainText = decrypt(cipherText, self.userKey)
        # Преобразование расшифрованного текста обратно в байты
        return codecs.decode(help_functions.reverse(help_functions.bitstring2hexstring(plainText)), 'hex')
